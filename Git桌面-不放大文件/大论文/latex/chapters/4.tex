...
\section{问题分析与定义}
考虑一个典型的两方纵向联邦学习（Vertical Federated Learning, VFL）场景，其中涉及两个参与方通过各自独立的特征集进行合作学习，在保证数据隐私和安全的前提下，共同训练一个机器学习模型\citep{yang2019federated}。参与方包括 Party A 和 Party B，其中只有一方拥有标签。首先，Party A 拥有数据集：
\begin{equation}
...
定义 1 特征列秩向量：设参与方 A 的特征空间为 $ \Phi^A = \{\varphi^A_1, \varphi^A_2, ..., \varphi^A_{m_A}\} $，$ m_A $ 表示 A 方的特征维数，即 A 方拥有 $ m_A $ 个特征。$ \varphi^A_p \in \mathbb{R}^{n_{al}} $ 表示 A 方第 $ p $ 个特征在对齐样本集 $ \mathcal{D}^A_{al} $ 上的观测向量，$ \varphi^A_p = [\varphi^A_{p1}, \varphi^A_{p2}, ..., \varphi^A_{pn_{al}}] $ 表示该特征在所有对齐样本上的取值，$ n_{al} $ 为对齐样本的数量。
...
\begin{table}[!htb]
	...
	\begin{tabular}{p{13.2cm}}
		...
		\midrule[0.75pt]
		\makecell[l]{\wuhao \textbf{输入:} $X_{al}^A$: 参与方A的对齐数据, $X_{nl}^A$: 参与方A的非对齐数据,} \\
		\makecell[l]{\wuhao \qquad $X_{al}^B$: 参与方B的对齐数据，$X_{nl}^B$: 参与方B的非对齐数据,} \\
		\makecell[l]{\wuhao \qquad $y$: 对齐数据的标签, $\alpha$: 置信度阈值, $T$: 迭代次数，$k$: 每次迭代选取比例。 }\\
%		\makecell[l]{\wuhao \quad} \\
		\makecell[l]{\wuhao \textbf{输出:} $\mathbf{y}^{\text{pseudo}}$} \\
%		\makecell[l]{\wuhao 1: 初始化联邦模型 $f_\theta$} \\
		\makecell[l]{\wuhao 2: $\mathcal{D}_{L}^{(0)} = \{X_{al}^A, X_{al}^B, y\}$} \\
		\makecell[l]{\wuhao 3: $\mathcal{D}_{U}^{(0)} = \{X_{nl}^A, X_{nl}^B\}$} \\
		\makecell[l]{\wuhao 4: \textbf{for} $t = 1$ \textbf{to} $T$ \textbf{do}} \\
%		\makecell[l]{\wuhao 5: \quad \textbf{联邦模型训练:}} \\
		\makecell[l]{\wuhao 6: \quad $\theta^{(t)} \leftarrow \arg\min_\theta \sum_{(\mathbf{x}^A,\mathbf{x}^B,y)\in\mathcal{D}_L^{(t)}} \ell(f_\theta(\mathbf{x}^A,\mathbf{x}^B), y)$} \\
%		\makecell[l]{\wuhao 7: \quad \textbf{置信度计算:}} \\
		\makecell[l]{\wuhao 8: \quad 对 $\forall (\mathbf{x}^A_j,\mathbf{x}^B_j) \in \mathcal{D}_U^{(t)}$，计算置信度:} \\
		\makecell[l]{\wuhao 9: \quad $s_j = \begin{cases} 
				\max_{c} \mathbb{P}(y=c|f_{\theta^{(t)}}(\mathbf{x}^A_j,\mathbf{x}^B_j)) & \text{分类任务} \\
				1 - \frac{|\hat{y}_j - \mu_t|}{\sigma_t} & \text{回归任务}
			\end{cases}$} \\
		\makecell[l]{\wuhao 10: \quad 其中 $\mu_t,\sigma_t$ 为当前预测均值和标准差} \\
%		\makecell[l]{\wuhao 11: \quad \textbf{样本选择:}} \\
		\makecell[l]{\wuhao 12: \quad $\mathcal{C}^{(t)} = \{(\mathbf{x}^A_j,\mathbf{x}^B_j) \in \mathcal{D}_U^{(t)} | s_j \geq \alpha\}$} \\
		\makecell[l]{\wuhao 13: \quad 按置信度排序后选取前 $k$ 比例样本:} \\
		\makecell[l]{\wuhao 14: \quad $\mathcal{S}^{(t)} = \text{TopK}(\mathcal{C}^{(t)}, k)$} \\
%		\makecell[l]{\wuhao 15: \quad \textbf{伪标签生成:}} \\
		\makecell[l]{\wuhao 16: \quad $\forall (\mathbf{x}^A_j,\mathbf{x}^B_j) \in \mathcal{S}^{(t)}, \hat{y}_j = \begin{cases}
				\arg\max_c P(y=c|f_{\theta^{(t)}}(\mathbf{x}^A_j,\mathbf{x}^B_j)) & \text{分类} \\
				f_{\theta^{(t)}}(\mathbf{x}^A_j,\mathbf{x}^B_j) & \text{回归}
			\end{cases}$} \\
%		\makecell[l]{\wuhao 17: \quad \textbf{数据集更新:}} \\
		\makecell[l]{\wuhao 18: \quad $\mathcal{D}_L^{(t+1)} \leftarrow \mathcal{D}_L^{(t)} \cup \{(\mathbf{x}^A_j,\mathbf{x}^B_j,\hat{y}_j)\}_{j\in\mathcal{S}^{(t)}}$} \\
		\makecell[l]{\wuhao 19: \quad $\mathcal{D}_U^{(t+1)} \leftarrow \mathcal{D}_U^{(t)} \setminus \mathcal{S}^{(t)}$} \\
		\makecell[l]{\wuhao 20: \textbf{end for}} \\
		\makecell[l]{\wuhao 21: $\mathbf{y}^{\text{pseudo}} = \left[ y, \bigcup_{t=1}^{T} \{\hat{y}_j\}_{j \in \mathcal{S}^{(t)}} \right]$} \\
		\makecell[l]{\wuhao 22: \textbf{return} $\mathbf{y}^{\text{pseudo}}$} \\
		\bottomrule[1.5pt]
\end{tabular}
...
\begin{equation}
	\mathcal{L}_D = \mathbb{E}[\log D_B(X^B)] + \mathbb{E}[\log(1 - D_B(\tilde{X}^B))]
\end{equation}
进行训练；与此同时，生成器则通过以下目标函数来更新参数  
\begin{equation}
	\mathcal{L}_G = \mathbb{E}[\log D_B(\tilde{X}^B)] + \lambda \cdot \text{MSE}([X^A], E(\tilde{X}^B))
\end{equation}
